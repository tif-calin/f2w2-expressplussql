...
app.post('/api/books', async (req, res) => {
  try {
    ...
    const data = await client.query(`
      ...
      VALUES ($1, $2, $3, $4, $5)
      RETURNING id, isbn13, title, image, year, user_id as "userId";
    `, [book.isbn13, book.title, book.image, book.year, book.userId]
    );
...
app.get('/api/books/name/:name', async (req, res) => {
  try {
    const data = await client.query(`
      SELECT  id,
              isbn13,
...
app.get('/api/users/:user_id/books', async (req, res) => {
  try {
    const data = await client.query(`
      SELECT  id,
              isbn13,
